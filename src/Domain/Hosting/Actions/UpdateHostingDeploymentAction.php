    ...
    public function execute(UpdateHostingDeploymentDto $dto): void
    {
        ...
        switch ($provider->type) {
            case ProviderType::SITEBUILDER:
                $mailProvider = $this->providerRepository->getByType(ProviderType::MAILONLY, ProviderSlug::from($dto->mailProvider ?? ''));
            ...
            case ProviderType::MAILONLY:
                $mailProvider = $this->providerRepository->getByType(ProviderType::MAILONLY, ProviderSlug::from($dto->provider));
                $hostingDeployment->mail_only_provider_id   = $mailProvider->id;
                ...
                break;
            default:
                $hostingDeployment->plesk_customer_username       = $provider->slug === ProviderSlug::PLESK ? $dto->username : null;
                $hostingDeployment->directadmin_customer_username = $provider->slug === ProviderSlug::DIRECTADMIN ? $dto->username : null;
                $hostingDeployment->plesk_customer_id             = $provider->slug === ProviderSlug::PLESK ? $dto->pleskCustomerId : null;
                $hostingDeployment->server_id                     = $server->type === ServerType::DIRECTADMIN || $server->type === ServerType::PLESK ? $server->id : null;
