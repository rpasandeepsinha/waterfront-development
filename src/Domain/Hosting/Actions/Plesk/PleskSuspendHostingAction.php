...
class PleskSuspendHostingAction
{
    ...
    public function execute(Server $server, HostingDeployment $hostingDeployment): Result
    {
        $this->logger->info(
            sprintf(
                'Suspending hosting package for subscription: %s on plesk server id: %s ',
        ...
        $result = $this->hostingPackageClient->disable($parameters);

        $this->logger->info(
            sprintf(
                'Suspending hosting package for subscription: "%s" on plesk server id: "%s" with Plesk XML response: %s',
                $hostingDeployment->subscription->uuid,
                $server->id,
