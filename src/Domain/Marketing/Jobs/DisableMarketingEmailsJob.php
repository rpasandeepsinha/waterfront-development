...
/** @internal */
class DisableMarketingEmailsJob extends AbstractQueueableJob
{
     ...
     * @throws Exception
     */
    public function handle(ContactsClient $hubspotContactsClient, HubspotEventRepository $hubspotEventRepository): void
    {
        $event = $hubspotEventRepository->createPendingEvent($this->customer->id, 'Disable marketing emails');

        try {
            $contact = $hubspotContactsClient->findBySandwaveUuid($this->customer->uuid);
            if ($contact !== null && filter_var($contact->marketingOptIn, FILTER_VALIDATE_BOOLEAN)) {
                $hubspotContactsClient->setMarketable($contact, false);
            }
