...
class RemoteDomainNotFoundException extends RuntimeException
{
    public function __construct(Subscription $subscription, ProviderSlug $driver, Throwable $previous)
    {
        $message = sprintf(
            'Domain {%s} not found in backend {%s} and business unit {%s} for customer with id {%d} error message from remote: %s',
            $subscription->domain,
            $driver->value,
