...
class ValidationService
{
    ...
    public function validate(
        ManualMigrationValidateRequest $request,
        Customer $customer,
    ): ValidatedDomain|ValidatedHosting {
    ...
    public function processDomain(array $ferryValidationResults, string $domain): ValidatedDomain
    {
        ...
        $options = $this->calculateDomainSelectableOptions(
            $zoneInPowerDns,
        ...
        return [
            'nameservers' => [
                new MigrationOption(
                    ...
                    description: $this->translator->translate(
                        'manual-migration.validation.option.do-nothing.description',
                    ),
                ...
                new MigrationOption(
                    ...
                    selected: false,
                    description: $this->translator->translate(
                        'manual-migration.validation.option.nameservers.update-new.description',
                    ),
            ...
            'dnssec' => [
                ...
                new MigrationOption(
                    ...
                    selected: false,
                    description: $this->translator->translate(
                        'manual-migration.validation.option.dnssec.enable.description',
                    ),
            ...
            'dns' => [
                ...
                new MigrationOption(
                    ...
                    selected: false,
                    description: $this->translator->translate(
                        'manual-migration.validation.option.dns.update-native.description',
                    ),
                ...
                new MigrationOption(
                    ...
                    selected: false,
                    description: $this->translator->translate(
                        'manual-migration.validation.option.dns.default-template.description',
                    ),
                    optionAvailable: $zoneInPowerDns || ! $zoneIsNative,
                ),
                new MigrationOption(
                    ...
                    selected: false,
                    description: $this->translator->translate(
                        'manual-migration.validation.option.dns.new-empty.description',
                    ),
                    optionAvailable: $zoneInPowerDns || ! $zoneIsNative,
                ),
            ],
    ...
    public function filterImportantErrors(array $ferryValidationResults): array
    {
        ...
        $errorList = [
            MigrationValidation::PIPE_NOT_IMPLEMENTED->value => $this->translator->translate(
                'manual-migration.validation.error.pipe-not-implemented',
            ),
            MigrationValidation::DEFAULT_VALIDATION->value => $this->translator->translate(
                'manual-migration.validation.error.default-validation',
            ),
            MigrationValidation::SUBSCRIPTION_VOLUME_DISCOUNT_PRODUCT_INCORRECT->value => $this->translator->translate(
                'manual-migration.validation.error.subscription-volume-discount-product-incorrect',
            ),
            MigrationValidation::SUBSCRIPTION_ALREADY_MIGRATED->value => $this->translator->translate(
                'manual-migration.validation.error.subscription-already-migrated',
            ),
            MigrationValidation::DOMAIN_MIGRATION_DRIVER_CREDENTIALS_FAILED->value => $this->translator->translate(
                'manual-migration.validation.error.domain-migration-driver-credentials-failed',
            ),
            MigrationValidation::DOMAIN_MIGRATION_BUSINESS_UNIT_FAILED->value => $this->translator->translate(
                'manual-migration.validation.error.domain-migration-business-unit-failed',
            ),
            MigrationValidation::DOMAIN_MIGRATION_INVALID_PHONE->value => $this->translator->translate(
                'manual-migration.validation.error.domain-migration-invalid-phone',
            ),
            MigrationValidation::DNS_CONFIGURATION_ZONE_UNEXPECTED_EXCEPTION->value => $this->translator->translate(
                'manual-migration.validation.error.dns-configuration-zone-unexpected-exception',
            ),
            MigrationValidation::DNSSEC_ZONE_UNEXPECTED_EXCEPTION->value => $this->translator->translate(
                'manual-migration.validation.error.dnssec-zone-unexpected-exception',
            ),
        ];

        return array_filter(
            $errorList,
            fn (string $errorKey) => in_array(MigrationValidation::from($errorKey), $ferryValidationResults, true),
     ...
     * @param array<MigrationValidation> $ferryValidationResults
     */
    private function processHosting(array $ferryValidationResults): ValidatedHosting
    {
        $errors = $this->filterImportantErrors($ferryValidationResults);
