...
class HandleTechnicalDomainBulkPayloadJob extends AbstractQueueableJob
{
    ...
    public function handle(
        ...
        ExecuteTechnicalDomainMigrationAction $executeTechnicalDomainMigrationAction,
    ): void {
        $amount = count($this->domainPayloads);

        $logger->info('Starting to insert bulk technical domain migration jobs.', [
        ...
        foreach ($this->domainPayloads as $key => $domainPayload) {
            ...
            if (! $customer instanceof Customer) {
                $logger->warning('Unknown customer id in ferry technical domain bulk migrations proxy job. Skipping this payload', [
                    LoggingContextKeys::QUEUE_JOB_ID => $this->job?->getJobId(),
            ...
            if (! $subscriptions->isEmpty()) {
                $executeTechnicalDomainMigrationAction->execute($subscriptions, $rawSubscriptionData);
        ...
        $logger->info('Completed inserting bulk technical domain migration jobs.', [
            LoggingContextKeys::QUEUE_JOB_ID => $this->job?->getJobId(),
            LoggingContextKeys::META => [
                'customer_amount' => $amount,
            ],
