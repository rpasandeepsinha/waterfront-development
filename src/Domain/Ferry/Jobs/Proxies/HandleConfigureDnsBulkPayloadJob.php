...
class HandleConfigureDnsBulkPayloadJob extends AbstractQueueableJob
{
    ...
    public function handle(
        ...
        ExecuteConfigureDnsAction $executeConfigureDnsAction,
    ): void {
        $amount = count($this->configureDnsPayloads);

        $logger->info('Starting to insert bulk configure dns migration jobs.', [
        ...
        foreach ($this->configureDnsPayloads as $key => $configureDnsPayload) {
            ...
            if (! $customer instanceof Customer) {
                $logger->warning('Unknown customer id in ferry configure dns bulk migrations proxy job. Skipping this payload', [
                    LoggingContextKeys::QUEUE_JOB_ID => $this->job?->getJobId(),
            ...
            if (! $subscriptions->isEmpty()) {
                $executeConfigureDnsAction->execute($subscriptions);
        ...
        $logger->info('Completed inserting bulk configure dns migration jobs.', [
            LoggingContextKeys::QUEUE_JOB_ID => $this->job?->getJobId(),
            LoggingContextKeys::META => [
                'customer_amount' => $amount,
            ],
