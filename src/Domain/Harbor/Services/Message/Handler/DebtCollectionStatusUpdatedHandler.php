...
class DebtCollectionStatusUpdatedHandler
{
    ...
    private function handleSubscriptionStatusInArrears(Subscription $subscription): void
    {
        ...
        if (in_array($subscription->administrative_status, AdministrativeStatus::getIneligibleForSuspension(), true)) {
            $this->logger->debug('No action performed because subscription is not in an eligible state for suspension', $loggingContext);
            return;
        }

        if ($subscription->administrative_status === AdministrativeStatus::CANCELED->value && $subscription->cancel_reason === SubscriptionCancelReason::REASON_BAD_DEBT) {
            $this->logger->debug('Re-activating auto cancelled subscriptions (caused by bad debt) must be done manually by CS.', $loggingContext);
            return;
        }
        $this->logger->info('Auto suspending subscription {subscription.id} because debt collection status is now in arrears', $loggingContext);
