...
class PriceList extends Collection
{
     ...
     */
    #[Deprecated(message: "Don't use this function, it's only here for legacy reasons. You want `getProductPrice()` below.")]
    public function getPrice(string $productSlug, int $contractPeriod, int $billingPeriod, ProductPriceType $type): Price
    {
        ...
        if ($price === null) {
            throw new PriceResolvingException("Cannot resolve price for product: slug={$productSlug} contract_period={$contractPeriod} billing_period={$billingPeriod} type={$type->value}");
        }
        return $price;
    }
    ...
    public function getProductPrice(string $productSlug, int $contractPeriod, int $billingPeriod): Price
    {
        try {
            ...
            return $product->prices->where('contractPeriod', $contractPeriod)->where('billingPeriod', $billingPeriod)->where('type', ProductPriceType::REGISTRATION)->firstOrFail();
        } catch (ItemNotFoundException $e) {
            throw new ItemNotFoundException(
                sprintf('No base price found for product with slug %s, contract period %d, billing period %d', $productSlug, $contractPeriod, $billingPeriod),
                $e->getCode(),
                $e
            );
        }
