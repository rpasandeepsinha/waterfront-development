...
readonly class ConnectorConfig
{
    public function __construct(
        ...
        Assert::regex($this->authUrl, pattern: '/^http(s)?:\/\//', message: 'The Puzzel auth URL must start with http:// or https://');
        Assert::regex($this->apiUrl, pattern: '/^http(s)?:\/\//', message: 'The Puzzel api URL must start with http:// or https://');
        Assert::stringNotEmpty($this->clientId, message: 'The Puzzel client id must be a valid UUID');
        Assert::regex($this->clientSecret, pattern: '/^[a-zA-Z0-9]+$/', message: 'The Puzzel client secret syntax is not valid. only letters and digits are allowed');
