...
class NovaProductPromotionsResource extends Resource
{
    ...
    public function fields(NovaRequest $request): array
    {
        ...
        foreach (ProductPromotionPlatform::cases() as $platform) {
            $platformOptions[$platform->value] = self::translate('product_promotion.platform.' . strtolower($platform->name));
        }
        ...
        return [
            ...
            NovaProductSelectField::makeForProductId('product_id', null, true)
                ->onlyOnForms()
                ->required(),
            Text::make(
                self::translate('subscription.attributes.start-end_date'),
                fn () => $this->resource->start_date->format(DateTimeFormat::DUTCH)
                    . ' - '
            ...
            )->exceptOnForms()->displayUsing(fn () => $this->resource->placement_url),
            Select::make(
                self::translate('product_promotion.create.label.placement_url'),
                'placement_url'
            )
            ...
            URL::make('destination_url', 'call_to_action->destination_url')->displayUsing(fn () => Arr::get($this->resource->call_to_action, 'destination_url', 'N/A'))->exceptOnForms()->hideFromIndex(),
            Text::make(
                ...
                ->onlyOnForms()
                ->required()->rules('url', 'url:https'),
            Number::make(
                self::translate('product_promotion.create.label.weight'),
                'weight'
            )->rules(['required', 'between: 1,100']),
            Date::make(self::translate('subscription.attributes.start_date'), 'start_date')->sortable()->hideFromDetail(),
