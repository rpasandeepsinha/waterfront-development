...
class NovaProductSelectField
{
            ...
            ->options(
                function () use ($productGroup, $orderable): Collection {
                    $builder = Product::query()
                        ->selectRaw(
                            'products.name as productname, products.uuid as productuuid, product_groups.name as productgroupname',
                        )
    ...
    public static function makeForProductId(
        string $attribute,
        ?ProductGroup $productGroup = null,
        ?bool $orderable = null,
    ): Select {
        ...
        return Select::make($translator->translate('subscription.relations.product'), $attribute)->options(
            function () use ($productGroup, $orderable): Collection {
                $builder = Product::query()
                    ->selectRaw(
                        'products.name as productname, products.id as productid, product_groups.name as productgroupname',
                    )
                    ->withoutGlobalScope('order')
                    ->join('product_groups', 'product_groups.id', '=', 'products.product_group_id')
                    ->orderByRaw('LOWER(product_groups.name), LOWER(products.name)');
