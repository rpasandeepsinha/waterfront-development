...
class NovaProvisionRequestResource extends Resource
{
    ...
    public function fields(NovaRequest $request): array
    {
        return [
            ID::make(),
            Text::make('UUID', 'uuid')->copyable()->onlyOnDetail()->readonly(),
            Text::make(self::translate('provisioning-request.attributes.name'), 'request_name')->readonly(),
            Text::make(self::translate('provisioning-request.attributes.type'), 'request_type')->readonly(),
            BelongsTo::make(self::translate('subscription.singular'), 'subscription', NovaSubscriptionResource::class)
                ...
                ->displayUsing(fn (NovaSubscriptionResource $subscription) => sprintf(
                    '%s (%s - %s)',
                    $subscription->domain, //@phpstan-ignore-line argument.type
                    $subscription->product->productGroup->name, //@phpstan-ignore-line argument.type
                    $subscription->product->name, //@phpstan-ignore-line argument.type
                )),

            StatusBadgeConverter::createProvisionStatusBadge(self::translate(
                'provisioning-request.attributes.last-status',
            ))->resolveUsing(fn (mixed $value, mixed $resource, mixed $attribute) => $this->resource
                ->result()
                ->orderBy('created_at', 'desc')
                ->first()
                ?->status
                ->value ?? StatusBadgeConverter::MISSING),
            ...
            DateTime::make(self::translate('provisioning-request.attributes.created_at'), 'created_at')
                ->onlyOnDetail()
                ->readonly(),
            DateTime::make(self::translate('provisioning-request.attributes.updated_at'), 'updated_at')->readonly(),
