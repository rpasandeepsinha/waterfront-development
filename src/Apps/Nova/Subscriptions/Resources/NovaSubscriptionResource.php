...
class NovaSubscriptionResource extends Resource
{
    ...
    public function fields(NovaRequest $request): array
    {
        ...
        return [
            ...
            BelongsTo::make(
                ...
                ->canSee(
                    fn (Request $request): bool => $this->canSeeSlugged(ProductGroupType::MICROSOFT_365, $subscription),
                )
                ->onlyOnDetail(),
            BelongsTo::make(
            ...
            Number::make(
                ...
                fn () => $this->resource->contract_period . ' - ' . $this->resource->billing_period,
            )->onlyOnDetail(),
            ...
            NovaSubscriptionAdministrativeStatusSelectField::makeForEditing()->displayUsingLabels()->required(),
            ...
            Text::make(
                ...
                ->canSee(
                    fn (Request $request): bool => $this->canSeeSlugged(ProductGroupType::EXTENSION, $subscription),
                )
                ->onlyOnDetail(),
            ...
            Text::make(
                ...
                fn () => (
                    $this->resource->start_date->format(DateTimeFormat::DUTCHNOTIME)
                    . ' - '
            ...
            Text::make(
                ...
                function () {
                    $cancel_date = $this->resource->cancel_date === null
                        ? ''
                        : $this->resource->cancel_date->format(DateTimeFormat::DUTCH);
                    $termination_date = $this->resource->termination_date === null
                        ? ''
                        : $this->resource->termination_date->format(DateTimeFormat::DUTCH);

                    return $cancel_date . ' - ' . $termination_date;
                },
            )
                ->hideFromDetail(
                    fn () => $this->resource->termination_date === null && $this->resource->cancel_date === null,
                )
                ->hideFromIndex()
                ->hideWhenCreating()
                ->hideWhenUpdating(),
            Text::make(
                ...
                fn () => self::translate(
                    'cancel_subscriptions.reason.' . strtolower($this->resource->cancel_reason->value ?? ''),
                ),
            )
                ->canSee(fn (): bool => $this->resource->cancel_reason !== null)
                ->onlyOnDetail(),
            ...
            NovaBoolField::make(self::translate('nova-resource-labels.subscription.relation.note'), 'notes')
                ->hideWhenCreating()
                ->hideWhenUpdating()
                ->hideFromDetail(fn () => $this->resource->notes()->count() === 0),
            NovaBoolField::make(self::translate('transfer.relations.is_in_transfer'), 'in_transfer')
                ->onlyOnDetail()
                ->hideFromDetail(fn () => $transferService->hasOpenTransfer($this->resource) === false),
            ...
            Text::make(
                ...
                function (): ?string {
                    ...
                    if ($order instanceof Order) {
                        ...
                        return (
                            "<a href='"
                            . sprintf('/nova/resources/nova-order-resources/%s', $order->id)
            ...
            HasOne::make(
                ...
                fn (Request $request): bool => $this->canSeeSlugged(ProductGroupType::RESELLER_HOSTING, $subscription),
            ),
            ...
            Text::make(
                ...
                ->displayUsing(fn () => $this->resource->category?->name->value ?? 'N/A')
                ->canSee(fn (Request $request): bool => $this->resource->category !== null)
                ->hideFromIndex(),
            ...
            HasMany::make(
                ...
                ->onlyOnDetail()
                ->canSee(fn (Request $request): bool => $subscription?->parent !== null),
            ...
            HasMany::make(
                ...
                ->onlyOnDetail()
                ->canSee(fn (Request $request): bool => $subscription?->children->count() > 0),
    ...
    public function actions(NovaRequest $request): array
    {
        ...
        return [
            ...
            $this->resolveAction(NovaCallAdministrativelyExpireSubscriptions::class)->canSee(
                fn () => $this->canPerformRestrictedSubscriptionLifecycleActions(),
            ),
            $this->resolveAction(NovaCallTerminateSubscriptions::class)->canSee(
                fn () => $this->canPerformRestrictedSubscriptionLifecycleActions(),
            ),
    ...
    public function fieldsForUpdate(NovaRequest $request): array
    {
        return [
            BelongsTo::make(
                ...
                ->searchable()
                ->relatableQueryUsing(
                    fn (NovaRequest $request, Builder $query): Builder => $query->withoutEagerLoads(),
                ),
    ...
    protected static function afterValidation(NovaRequest $request, Validator $validator): void
    {
        ...
        if ($subscription->net_price !== $newPrice) {
            $pricePersistService->persistCustomPrice(
                ...
                $newPrice,
                true,
                CustomPriceReasonType::MANUAL_NOVA_OVERRIDE,
            );
        }
    }
    ...
    private function canSeeSlugged(ProductGroupType $productType, ?Subscription $subscription): bool
    {
        return (
            $subscription === null
