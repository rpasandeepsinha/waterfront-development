...
Route::prefix('subscriptions/')
    ...
    ->group(function (): void {
        ...
        Route::get('bydomain/{domain}', [SubscriptionsController::class, 'listSubscriptionsForDomain'])->name(
            'list.for.domain',
        );
        Route::post('cancel-and-credit', [CancelSubscriptionsController::class, 'cancelAndCredit'])->name(
            'cancel-and-credit',
        );
        Route::post('cancel-and-credit/check', [CancelSubscriptionsController::class, 'check'])->name(
            'cancel-and-credit.check',
        );
        Route::get('{identifier}', [SubscriptionsController::class, 'show'])->name('subscription.show');
        Route::delete('mutations/{subscriptionMutation}', [SubscriptionsController::class, 'deleteMutation'])->name(
            'delete.mutation',
        );
        ...
        Route::prefix('{subscription}')
            ...
            ->group(function (): void {
                ...
                Route::get('resellerHostingDeployment', [
                    ResellerHostingDeploymentController::class,
                    'deployment',
                ])->name('reseller-hosting-deployment');
                ...
                Route::get('product-transfers', [ProductTransferController::class, 'productTransfers'])->name(
                    'product.transfers',
                );
                Route::get('migration', [SubscriptionsController::class, 'migration'])->name('migration');
                Route::get('provisioning-requests', [SubscriptionsController::class, 'provisioningRequests'])->name(
                    'provisioning-requests',
                );
                Route::put('update-contract', [
                    SubscriptionsController::class,
                    'updateContractPeriodWithDiscount',
                ...
                Route::post('assign-employee', [SubscriptionsController::class, 'assignEmployee'])->name(
                    'assign-employee',
                );
                Route::post('assign-category', [SubscriptionsController::class, 'assignCategory'])->name(
                    'assign-category',
                );
                Route::get('invoice/prefill', [SubscriptionsController::class, 'invoicePrefill'])->name(
                    'invoice.prefill',
                );
                Route::post('invoice', [SubscriptionsController::class, 'createInvoice'])->name('invoice.create');
                Route::post('one-time-services', [OneTimeServiceController::class, 'store'])->name(
                    'one-time-services.store',
                );
                Route::post('one-time-services/preview', [OneTimeServiceController::class, 'preview'])->name(
                    'one-time-services.preview',
                );
            });
        ...
        Route::prefix('{subscription:uuid}')
            ->name('subscription.')
            ->group(function (): void {
                Route::post('add-domain-to-spam-experts', [
                    SubscriptionsController::class,
                    'addDomainToSpamExperts',
                ])->name('add.domain.to.spam.experts');
            });
    });
