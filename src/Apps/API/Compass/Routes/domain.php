...
Route::prefix('domain/{domain}')
    ->name('domain.')
    ->group(function (): void {
        Route::get('/contact', [DomainController::class, 'contact'])->name('contact');
        Route::post('/contact/sync', [DomainController::class, 'syncDomainContact'])->name('contact.sync');
        Route::get('/nameservers', [DomainController::class, 'nameservers'])->name('nameservers');
        Route::get('/dns-zone-from-source', [DomainController::class, 'fetchDnsZoneFromSource'])->name(
            'dns.from-source',
        );
