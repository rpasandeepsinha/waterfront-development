...
Route::prefix('customers')->name('customers.')->group(function (): void {
    ...
    Route::prefix('{customer:customer_number}')->group(function (): void {
        ...
        Route::get('subscriptions/list', [CustomersController::class, 'listSubscriptions'])->name('index.subscriptions');
        Route::get('one-time-services/list', [CustomersController::class, 'listOneTimeServices'])->name('one-time-services.list');
        ...
        Route::get('invoice-lines/unprocessed', [CustomersController::class, 'listUnprocessedInvoiceLines'])->name('invoice-lines.unprocessed');
        Route::post('invoice-lines/propagate', [CustomersController::class, 'propagateInvoiceLinesToHarbor'])->name('invoice-lines.propagate');
        ...
        Route::post('request-bu-invoices', [CustomersController::class, 'requestBuInvoices'])->name('requestBuInvoices');
        Route::post('create-mandate', [CustomersController::class, 'createMandate'])->name('create.mandate');
        Route::post('mark-as-abusive', [CustomersController::class, 'markAsAbusiveCustomer'])->name('mark.as.abuse');
        Route::post('refresh-vat-rate', [CustomersController::class, 'refreshVatRate'])->name('refresh.vat.rate');
        Route::get('volume-discounts/available', [CustomersController::class, 'listAvailableVolumeDiscounts'])->name('volume-discounts.available');
        Route::post('add-volume-discount', [CustomersController::class, 'addVolumeDiscount'])->name('add.volume.discount');
        ...
        Route::prefix('contacts')->name('contacts.')->group(function (): void {
            ...
            Route::put('{customerContact:uuid}/update', [CustomersController::class, 'updateContact'])->name('update');
        });
    });
});
