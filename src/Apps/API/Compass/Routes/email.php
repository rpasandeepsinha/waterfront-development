...
Route::prefix('email-template')->name('email-template.')->group(function (): void {
...
Route::prefix('email-history')->name('email-history.')->group(function (): void {
    Route::get('/{uuid}', [EmailController::class, 'indexEmailHistory'])->name('email-history');

    Route::prefix('{emailHistory}')->name('email-history.')->group(function (): void {
