...
class MigrationStateResource extends JsonResource
{
    ...
    public static function fieldDefinitions(): array
    {
        return [
            ...
            'reference_customer_number' => new FieldDefinition(
                null,
                fn (Subscription $s) => self::migratedCustomer($s)?->reference_customer_number,
            ),
            'reference_subscription_id' => new FieldDefinition(
                null,
                fn (Subscription $s) => $s->migratedSubscriptions->first()?->reference_subscription_id,
            ),
            'business_unit' => new FieldDefinition(
                null,
                fn (Subscription $s) => self::migratedCustomer($s)?->reference_name,
            ),
            ...
            'cancel_date' =>
                new FieldDefinition('cancel_date', fn (Subscription $s) => $s->cancel_date?->toW3cString()),
            'next_billing_date' =>
                new FieldDefinition('next_billing_date', fn (Subscription $s) => $s->next_billing_date->toW3cString()),
            ...
            'administrative_successful' => new FieldDefinition(
                null,
                fn (Subscription $s) => self::migratedCustomer($s)?->administrative_successful,
            ),
            'enable_invoicing' => new FieldDefinition(
                null,
                fn (Subscription $s) => self::migratedCustomer($s)?->enable_invoicing,
            ),
            'successful' => new FieldDefinition(null, fn (Subscription $s) => self::migratedCustomer($s)?->successful),
            'migrated_at' => new FieldDefinition(null, fn (Subscription $s) => self::migratedCustomer(
                $s,
    ...
    {
        return (
            $subscription->domainDeployment?->provider?->slug->value ?? $subscription->hostingDeployment?->provider?->slug->value ?? $subscription->hostingDeployment?->mailProvider?->slug->value ?? $subscription->hostingDeployment?->sitebuilderProvider?->slug->value
