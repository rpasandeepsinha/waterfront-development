...
class ListCustomerResource extends JsonResource
{
    ...
    public static function fieldDefinitions(): array
    {
        return [
            ...
            'anonymized_at'             => new FieldDefinition('anonymized_at', fn (Customer $c) => $c->anonymized_at?->toW3cString()),
            'created_at'                => new FieldDefinition('created_at', fn (Customer $c) => $c->created_at?->toW3cString()),
            'reference_customer_number' => new FieldDefinition(null, fn (Customer $c) => self::migration($c)?->reference_customer_number),
            ...
            'administrative_successful' => new FieldDefinition(null, fn (Customer $c) => self::migration($c)?->administrative_successful),
            'technical_successful'      => new FieldDefinition(null, fn (Customer $c) => self::migration($c)?->technical_successful),
            'billing_successful'        => new FieldDefinition(null, fn (Customer $c) => self::migration($c)?->billing_successful),
            ...
            'migrated_at'               => new FieldDefinition(null, fn (Customer $c) => self::migration($c)?->migrated_at?->toW3cString()),
        ];
    }
