    ...
    public function rules(): array
    {
        return [
            ...
            'promotions.*.platform' => [
                ...
                'required',
                'string',
                Rule::in(array_column(ProductPromotionPlatform::cases(), 'value')),
            ],
            ...
            'promotions.*.callToAction.title' => [
                'sometimes',
                'required',
                'string',
                Rule::exists(TranslationKey::class, 'key')->where('source', TranslationSource::COAST->value),
            ],
            'promotions.*.callToAction.button_text' => [
                'sometimes',
                'required',
                'string',
                Rule::exists(TranslationKey::class, 'key')->where('source', TranslationSource::COAST->value),
            ],
            'promotions.*.callToAction.description' => [
                'sometimes',
                'required',
                'string',
                Rule::exists(TranslationKey::class, 'key')->where('source', TranslationSource::COAST->value),
            ],
            'promotions.*.callToAction.price_description' => [
                'sometimes',
                'required',
                'string',
                Rule::exists(TranslationKey::class, 'key')->where('source', TranslationSource::COAST->value),
            ...
            'prices.*.additional_prices.*.type' => [
                'required',
                'string',
                Rule::in(array_column(
                    [PriceComponentType::INTRODUCTION, PriceComponentType::PROLONGATION, PriceComponentType::PROMOTION],
                    'value',
                )),
            ],
            ...
            'introduction_price_configuration.*.first_months_discount_period' => [
                'sometimes',
                'nullable',
                'integer',
                'min:0',
