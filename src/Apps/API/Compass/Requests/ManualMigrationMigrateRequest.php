 ...
 * @property ?string          $plesk_customer_id
 */
class ManualMigrationMigrateRequest extends FormRequest
{
    /**
     * @return mixed[]
     */
    public function rules(): array
    {
        ...
        $productRepo = Container::getInstance()->make(ProductRepository::class);
        ...
        $group = $productRepo->findProductByUuid($this->product_uuid)->productGroup->slug;

        return match ($group) {
            ProductGroupType::EXTENSION => array_merge($rules, $this->getDomainRules()),
            ProductGroupType::HOSTING => array_merge($rules, $this->getHostingRules()),
            default => $rules,
        };
    }
     ...
     * @return mixed[]
     */
    public function getHostingRules(): array
    {
        return [
    ...
    public function getDomainRules(): array
    {
        return [
            ...
            'options.*' => [Rule::enum(ManualMigrationOption::class), 'distinct'],
        ];
    }
}
