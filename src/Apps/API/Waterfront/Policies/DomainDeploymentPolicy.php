...
class DomainDeploymentPolicy
{
    ...
    public function assertCanUseDnsSec(DomainDeployment $domainDeployment): void
    {
        ...
        $specValue = $this->productSpecRepository->getStringValueOfSpecification(
            $product,
            ProductSpecName::DOMAIN_DNSSEC_ENABLED,
        );
        ...
        $list = $customer->subscriptions->filter(
            fn (Subscription $s) => (
                ! in_array(
                    ...
                    [AdministrativeStatus::EXPIRED->value, AdministrativeStatus::ARCHIVED->value],
                    true,
                )
