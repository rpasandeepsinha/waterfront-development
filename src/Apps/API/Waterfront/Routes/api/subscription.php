    ...
    ->group(
        function (): void {
            Route::prefix('{subscription:uuid}')->group(function (): void {
                ...
                Route::get('/get-potential-upgrades', [SubscriptionController::class, 'getPotentialUpgrades'])->name(
                    'potential.upgrades',
                );
                Route::get('/get-potential-downgrades', [
                    SubscriptionController::class,
                    'getPotentialDowngrades',
                ])->name('potential.downgrades');
            });

            Route::post('cancel', [SubscriptionController::class, 'cancel'])->name('cancel');
            Route::post('cancel/child-subscriptions', [SubscriptionController::class, 'cancelChildSubscription'])->name(
                'cancel.child-subscriptions',
            );
            Route::get('/overview', [SubscriptionController::class, 'indexOverview'])->name('index.overview');
            Route::get('/service-plus/eligible', [
                SubscriptionController::class,
                'getSubscriptionsEligibleForServicePlus',
            ])->name('service-plus.eligible');
            Route::prefix('/{subscription:uuid}/')->group(
                function (): void {
                    Route::get('show', [SubscriptionController::class, 'show'])->name('show');
                    Route::get('domain-related', [
                        SubscriptionController::class,
                        'getDomainAndCustomerRelationSubscriptions',
                    ])->name('domain-related');
                },
            );
