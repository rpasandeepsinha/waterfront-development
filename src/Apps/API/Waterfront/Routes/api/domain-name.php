...
Route::prefix('domain-name')
    ...
    ->group(
        function (): void {
            Route::get('/{domainDeployment:subscription_uuid}/transfer-code', [
                DomainNameController::class,
                'transferCode',
            ])->name('transfer-code');
            ...
            Route::prefix('{domain}')->group(
                function (): void {
                    ...
                    Route::post('disable-dnssec', [DomainNameController::class, 'disableDnssec'])->name(
                        'disable-dnssec',
                    );

                    Route::post('retry-provisioning', [DomainNameController::class, 'retryProvisioning'])->name(
                        'retry-provisioning',
                    );

                    Route::get('manual-dnssec-available', [DomainNameController::class, 'manualDnssecAvailable'])->name(
                        'manual-dnssec-available',
                    );
                },
            );
        },
