...
class CreateSiteInSiteBuilderServiceTest extends IntegrationTestCase
{
    ...
    public function createSiteCallsGateway(): void
    {
        ...
        $product = ProductFactory::new()->for(ProductGroupFactory::new()->hosting()->createOne())->createOne(['slug' => ProductType::SITEBUILDER]);
        ProductSpecFactory::new()->for($product)->createOne(['name' => ProductSpecName::BASEKIT_PACKAGE_REFERENCE, 'value' => 123]);
        ...
        $childSub = SubscriptionFactory::new()->for($customer)->for($addonProduct)->createOne(['parent_subscription_id' => $parentSub->id, 'domain' => 'child.com']);
        ...
        $provisingGateway->expects(self::once())->method('request')->with(self::callback(function (CreateSitebuilderRequest $request) use ($parentSub) {
