...
#[CoversClass(NovaUpdateMollieCustomerAction::class)]
class UpdateMollieCustomerActionTest extends IntegrationTestCase
{
    ...
    #[Test]
    public function updateMollieCustomerAction(): void
    {
        $this->app->bind(function (): MollieCustomerManager {
            $mock = self::createStub(MollieCustomerManager::class);

            $mock->method('updateCustomer')->willReturn($this->getMollieCustomerResponseDTO());

            return $mock;
        });

        $action = self::resolve(NovaUpdateMollieCustomerAction::class);
        ...
        self::assertSame('Updated Test', $data['name']);
    }
