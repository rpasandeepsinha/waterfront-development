...
class TerminateChildSubscriptionsTest extends IntegrationTestCase
{
    ...
    public function parentSubscriptionIsDispatchedToQueueForCanceledChildSubscription(): void
    {
        ...
        Queue::assertPushedOn(
            ...
            fn (TerminateSubscription $job) => $subscription->id === $job->subscription->id,
        );
    ...
    public function terminateMicrosoft365ChildSubscriptions(): void
    {
        ...
        $cancel_child_count = Subscription::where('administrative_status', AdministrativeStatus::EXPIRED->value)
            ->whereNotNull('parent_subscription_id')
            ->count();
        self::assertSame(7, $cancel_child_count);

        //Check if 15 active child subscriptions are in the database.
        $active_child_count = Subscription::where('administrative_status', AdministrativeStatus::ACTIVE->value)
            ->whereNotNull('parent_subscription_id')
            ->count();
        self::assertSame(15, $active_child_count);

        $microsoft365Service = self::createMock(Microsoft365Service::class);
        $microsoft365Service->expects(self::exactly(2))->method('modifyOrder');
        ...
        $cancel_child_count = Subscription::where('administrative_status', AdministrativeStatus::ARCHIVED->value)
            ->whereNotNull('parent_subscription_id')
            ->count();
        self::assertSame(0, $cancel_child_count);

        //Check if 7 deleted subscription are in the database.
        $deleted_child_count = Subscription::where('administrative_status', AdministrativeStatus::ARCHIVING->value)
            ->whereNotNull('parent_subscription_id')
            ->count();
        ...
        $active_parent_count = Subscription::where('administrative_status', AdministrativeStatus::ACTIVE->value)
