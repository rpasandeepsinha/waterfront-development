...
class OrderMutationTest extends IntegrationTestCase
{
    ...
    public function orderContractExtension(): void
    {
        ...
        $comProduct = new ProductFactory()->for($extensionGroup)
            ->createOne(['name' => '.com', 'slug' => 'extension_com']);

        new ProductSpecFactory()->for($comProduct)
            ->createOne(['name'  => 'domain.provider_id', 'value' => $domainProvider->id]);

        new ProductPriceComponentFactory()->for($comProduct)->registration()
            ->createOne(['billing_period' => 12, 'contract_period' => 12, 'price' => 120]);
        new ProductPriceComponentFactory()->for($comProduct)->prolongation()
            ->createOne(['billing_period' => 24, 'contract_period' => 24, 'price' => 120]);

        $productAdminFees = new ProductFactory()->administrationFees()->createOne();
        new ProductPriceComponentFactory()->registration()->administrationFee()->createOne(['product_id' => $productAdminFees->id]);
        ...
        $subscription = new SubscriptionFactory()->for($customer)->for($comProduct)->createOne([
    ...
    public function orderDowngrade(): void
    {
        ...
        new ProductPriceComponentFactory()->for($grootProduct)->registration()
            ->createOne(['billing_period' => 12, 'contract_period' => 12, 'price' => 120]);
        new ProductPriceComponentFactory()->for($grootProduct)->prolongation()
            ->createOne(['billing_period' => 24, 'contract_period' => 24, 'price' => 120]);

        new ProductPriceComponentFactory()->for($smallProduct)->registration()
            ->createOne(['billing_period' => 12, 'contract_period' => 12, 'price' => 120]);
        new ProductPriceComponentFactory()->for($smallProduct)->prolongation()
            ->createOne(['billing_period' => 12, 'contract_period' => 12, 'price' => 120]);

        new ProductAllowedChangeFactory()->downgradeChange()
        ...
        $customer = new CustomerFactory()->withAddress()->createOne(['payment_type' => 'credit', 'has_direct_debit' => true]);
        $sub = new SubscriptionFactory()->for($customer)->for($grootProduct)->createOne([
            'uuid' => 'fda0da0b-4678-4716-ae8b-b30c62139137',
            'billing_period' => 12,
            'contract_period' => 12,
        ]);
    ...
    public function orderDowngradeFailsWhenNoChangePathIsSet(): void
    {
        ...
        $customer = new CustomerFactory()->withAddress()->createOne(['payment_type' => 'credit', 'has_direct_debit' => true]);
        new SubscriptionFactory()->for($customer)->for($grootProduct)->createOne([
            'uuid' => 'fda0da0b-4678-4716-ae8b-b30c62139137',
            'billing_period' => 12,
            'contract_period' => 12,
        ]);
    ...
    public function orderDowngradeAndContractExtension(): void
    {
        ...
        new ProductPriceComponentFactory()->administrationFee()->createOne(['product_id' => $productAdminFees->id]);

        $json = (string) file_get_contents(__DIR__ . '/data/order_downgrade_and_contract_extension_payload.json');

        $customer = new CustomerFactory()->withAddress()->createOne(['payment_type' => 'credit', 'has_direct_debit' => true]);
        $hostingSub = new SubscriptionFactory()->for($customer)->for($grootProduct)->createOne([
            'uuid' => 'fda0da0b-4678-4716-ae8b-b30c62139137',
            'domain' => 'grootSub.com',
            'billing_period' => 12,
            'contract_period' => 12,
        ...
        $extensionSub = new SubscriptionFactory()->for($customer)->for($comProduct)->createOne([
