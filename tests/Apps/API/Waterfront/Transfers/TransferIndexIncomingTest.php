...
class TransferIndexIncomingTest extends IntegrationTestCase
{
    ...
    #[Test]
    public function indexIncoming(): void
    {
        ...
        [$incoming1, $incoming2, $timestamped] = $this->prepareIndexTransfers($subscriptions, $executedAt);

        $response = $this->actingAsCustomer($this->receiver)->json(
            'get',
        ...
        Assert::assertArraySubset([
            ...
            'rejected_at' => null,
            'started_at' => null,
            'created_at' => $incoming1->created_at?->toW3cString(),
            'updated_at' => $incoming1->updated_at?->toW3cString(),
        ], $transfers[0]);
        ...
        Assert::assertArraySubset([
            ...
            'rejected_at' => null,
            'started_at' => null,
            'created_at' => $incoming2->created_at?->toW3cString(),
            'updated_at' => $incoming2->updated_at?->toW3cString(),
        ], $transfers[1]);
    ...
    #[Test]
    public function indexIncomingReverse(): void
    {
        self::assertEmailsSend([
            MailTransferAcceptedSender::class,
            MailTransferAcceptedReceiver::class,
    ...
    {
        $product = new ProductFactory()->for(new ProductGroupFactory()->extension()->createOne())->createOne(['slug' => 'extension_com']);
        $product2 = new ProductFactory()->for(new ProductGroupFactory()->hosting()->createOne())->createOne(['slug' => 'hosting_zilver']);
        $product3 = new ProductFactory()->for(new ProductGroupFactory()->ssl()->createOne())->createOne(['slug' => 'hosting_platina']);
    ...
    private function prepareIndexTransfers(Collection $subscriptions, string $executedAt): array
    {
        ...
        $incoming1 = new TransferFactory()->createOne([
            'from_customer_id' => $this->fromCustomer->id,
            'to_customer_id' => $this->receiver->id,
            'uuid'           => 'aaaa-bbbb-cccc-dddd',
        ]);
        $incoming1->subscriptions()->save($subscriptions[0]);

        self::assertInstanceOf(Subscription::class, $subscriptions[1]);
        $incoming2 = new TransferFactory()->createOne([
            'from_customer_id' => $this->fromCustomer->id,
            'to_customer_id' => $this->receiver->id,
            'uuid'           => 'bbbb-cccc-dddd-eeee',
        ]);
        $incoming2->subscriptions()->save($subscriptions[1]);
        ...
        return [$incoming1, $incoming2, $timestamped];
    }
}
