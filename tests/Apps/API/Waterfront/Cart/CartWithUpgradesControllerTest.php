...
class CartWithUpgradesControllerTest extends IntegrationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        new ProductPriceComponentFactory()->for($this->hostingProductSilver)->registration()->createOne([
            'billing_period' => 12,
            'contract_period' => 12,
            'price' => 1000,
        ]);

        new SubscriptionFactory()->for($this->hostingProductSilver)->for($this->customer)->createOne([
        ...
        new ProductPriceComponentFactory()->for($this->upgradeProduct)->registration()
            ->createOne(['billing_period' => 12, 'contract_period' => 12, 'price' => 1000]);
        new ProductPriceComponentFactory()->for($this->upgradeProduct)->prolongation()
    ...
    public function shouldProcessCartWithUpgradeAndCalculateProRataWithoutVouchers(): void
    {
        ...
        $json = (string) file_get_contents(__DIR__ . '/data/cart_payload_with_upgrade_and_regular_product_without_vouchers.json');
    ...
    public function shouldProcessCartWithUpgradeAndAddonWithoutVouchers(): void
    {
        ...
        self::assertIsArray($payload);
        $this->withoutExceptionHandling()->actingAsCustomer($this->customer)
            ->postJson($this->generateRoute('partners.cart.calculate'), $payload)
            ->assertOk()
            ->assertJsonFragment(
                ['totalExclVatPrice' => 2291,
    ...
    public function shouldProcessCartWithUpgradeAndAddonAndNormalProductsWithoutVouchers(): void
    {
        ...
        $json = (string) file_get_contents(__DIR__ . '/data/cart_payload_with_upgrade_and_addon_and_normal_product_without_vouchers.json');
    ...
    public function shouldProcessCartWithUpgradeAndAddonAndNormalProductsWithVouchers(): void
    {
        ...
        $json = (string) file_get_contents(__DIR__ . '/data/cart_payload_with_upgrade_and_addon_and_normal_product_with_vouchers.json');

        $payload =  json_decode($json, true);

        self::assertIsArray($payload);

        VoucherFactory::new()->for($extension)->createOne(['code' => 'fietsen', 'amount' => 100, 'amount_type' => VoucherAmountType::FIXED, 'description' => '', 'display_name' => '']);
    ...
    public function shouldProcessCartWithUpgradeAndNormalProductAndCalculateProRataWithoutVouchers(): void
    {
        ...
        $this->actingAsCustomer($this->customer)
            ...
            ->assertOk()
            ->assertJsonFragment(
                ['productSlug' => $this->upgradeProduct->slug,
                    'priceExclVat' => 888]
            )
            ->assertJsonFragment(['vouchers' => []]);
    }
    ...
    public function shouldProcessCartWithUpgradeAndNormalProductAndCalculateProRataWithVoucherButShouldApplyProrataPrice(): void
    {
        ...
        $json = (string) file_get_contents(__DIR__ . '/data/cart_payload_with_upgrade_and_regular_product_with_vouchers.json');
        ...
        $this->actingAsCustomer($this->customer)
            ->postJson($this->generateRoute('partners.cart.calculate'), $payload)
            ->assertOk()
            ->assertJsonFragment(['fietsen', 'claimed_amount' => 10, 'productSlug' => $extensionPR->slug, 'priceExclVat' => 990, 'priceType' => UsedProductPriceType::VOUCHER_PRICE->value]);
    }
}
