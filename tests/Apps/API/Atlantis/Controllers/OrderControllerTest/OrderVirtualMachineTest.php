...
class OrderVirtualMachineTest extends IntegrationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->osProductWithoutSshKey = new ProductFactory()->for($cloudstackOsGroup)->createOne(['slug' => 'ubuntu-lts-20.04']);

        $this->osProductWithSshKey = new ProductFactory()->for($cloudstackOsGroup)->createOne(['slug' => 'ubuntu-lts-20.04-ssh']);
        new ProductSpecFactory()->for($this->osProductWithSshKey)->createOne([
            'name' => ProductSpecName::SSH_KEY_REQUIRED->value,
            'value' => 1,
        ]);

        new ProductPriceComponentFactory()->for($vpsProduct)->registration()->createOne(['price' => 120]);
        new ProductPriceComponentFactory()->for($vpsProduct)->createOne(['type' => PriceComponentType::PROMOTION, 'price' => 96]);

        new ProductPriceComponentFactory()->for($this->osProductWithoutSshKey)->registration()->createOne([
            'price' => 0,
            'billing_period' => 1,
            'contract_period' => 1,
        ]);

        new ProductPriceComponentFactory()->for($this->osProductWithSshKey)->registration()->createOne([
            'price' => 0,
            'billing_period' => 1,
            'contract_period' => 1,
        ]);
    ...
    public function orderVpsWithoutSshKey(): void
    {
        ...
        $orderData = json_decode((string) file_get_contents(__DIR__ . '/data/order_payload_vps.json'), true, 512, JSON_THROW_ON_ERROR);
    ...
    public function orderVpsWithSshKey(): void
    {
        ...
        /** @var array<mixed, mixed> $orderData */
        $orderData = json_decode((string) file_get_contents(__DIR__ . '/data/order_payload_vps_with_ssh_key.json'), true, 512, JSON_THROW_ON_ERROR);
