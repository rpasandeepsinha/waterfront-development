    ...
    public function getHostingSiteSendsCorrectXmlMessageAndReturnsStatusOk(): void
    {
        $mock = new MockHandler([
            ...
            function (RequestInterface $request, $options) {
                self::assertSame(
                    (string) file_get_contents(__DIR__ . '/data/plesk_email_forwarding_create_request.xml'),
                    (string) $request->getBody(),
                );
                ...
                return new Response(
                    200,
                    [],
                    (string) file_get_contents(__DIR__ . '/data/plesk_email_forwarding_create_response.xml'),
                );
            },
        ]);
