...
use Waterfront\Domain\Hosting\Interfaces\Hosting\Models\EmailSetCatchAll\Parameters;
...
class EmailSetCatchAllTest extends IntegrationTestCase
{
    #[Test]
    public function getHostingSiteSendsCorrectXmlMessageAndReturnsStatusOk(): void
    {
        $mock = new MockHandler([
            ...
            function (RequestInterface $request, $options) {
                self::assertSame((string) file_get_contents(__DIR__ . '/data/plesk_email_set_catch_all_request.xml'), (string) $request->getBody());
                return new Response(200, [], (string) file_get_contents(__DIR__ . '/data/plesk_email_set_catch_all_response.xml'));
            },
        ]);
        ...
        $data = require __DIR__ . '/data/pleskEmailSetCatchAllData.php';
        $parameters = Parameters::create($data);

        $result = $customerClient->setEmailCatchAll($parameters);
    ...
    public function forwardingCreateSuccess(): void
    {
        ...
        $data = require __DIR__ . '/data/pleskEmailSetCatchAllData.php';
        $parameters = Parameters::create($data);
        $result = $hostingClient->setEmailCatchAll($parameters);
